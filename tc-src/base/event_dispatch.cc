...
void CEventDispatch::AddTimer(callback_t callback, void *user_data,
    ...
    for (it = timer_list_.begin(); it != timer_list_.end(); it++) {
        ...
        if (pItem->callback == callback && pItem->user_data == user_data) {
            pItem->interval = interval;//更新定时器间隔
            pItem->next_tick = GetTickCount() + interval;//更新下次触发时间
            return;
        }
    }
    //不存在相同的定时器，创建新的定时器
    ...
    pItem->user_data = user_data;//设置用户数据
    pItem->interval = interval;//设置定时器间隔
    pItem->next_tick = GetTickCount() + interval;//设置下次触发时间
...
void CEventDispatch::_CheckTimer() {
    uint64_t curr_tick = GetTickCount();//获取当前时间
    list<TimerItem *>::iterator it;
    ...
    for (it = timer_list_.begin(); it != timer_list_.end();) {
        ...
        if (curr_tick >= pItem->next_tick) {
            pItem->next_tick += pItem->interval;//更新下次触发时间
