 ...
 * //等待线程池结束
 * tpool.waitForAllDone(1000);      //参数<0时,
 * 表示无限等待(注意有人调用stop也会推出)
 * //此时: 外部需要结束线程池是调用
 * tpool.Stop();
 * 注意:
 * ThreadPool::exec执行任务返回的是个future, 因此可以通过future异步获取结果,
 * 比如: int testInt(int i)
 * {
 *     return i;
 * }
  ...
  public:
    ...
    */
    ...
    auto Exec(int64_t timeoutMs, F &&f, Args &&...args)
        ...
        int64_t expireTime =
            (timeoutMs == 0 ? 0 : TNOWMS + timeoutMs); // 获取现在时间
        //定义返回值类型
