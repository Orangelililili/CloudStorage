/*
api_common.cc 是业务层的公共工具库：
统一管理配置、提供 Redis/MySQL 操作封装、FastDFS 命令封装、字符串格式化，
让各个 api_xxx.cc 只关注自身业务逻辑，避免重复代码。
*/
...
// 本函数是用来格式化字符串的，用于将格式化字符串转换为std::string
// 参数：
// format：格式化字符串
// args：可变参数
// 返回值：格式化后的字符串
// 示例：
// std::string str = FormatString("Hello, %s!", "world");
// 输出：Hello, world!
