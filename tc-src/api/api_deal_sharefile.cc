...
int handlePvFile(string &md5, string &filename) {
    ...
    AUTO_REL_DBCONN(db_manager, db_conn);//自动归还mysql数据库到连接池
    CacheManager *cache_manager = CacheManager::getInstance();
    CacheConn *cache_conn = cache_manager->GetCacheConn("token");
    AUTO_REL_CACHECONN(cache_manager, cache_conn);//自动归还redis连接到连接池
    ...
    LogInfo("执行: {}", sql_cmd);
    CResultSet *result_set = db_conn->ExecuteQuery(sql_cmd);//执行sql语句，返回结果集
    ...
    ret2 = cache_conn->ZsetExit(FILE_PUBLIC_ZSET, fileid);//判断元素是否在集合中
    if (ret2 == 1) //存在
    {              //===3、如果存在，有序集合score+1
