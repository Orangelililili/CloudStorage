...
int handleShareFile(string &user, string &md5, string &filename) {
    ...
    d)如果此文件没有被分享，mysql保存一份持久化操作(mysql操作)，防止重启redis丢失
...
int handleDeleteFile(string &user, string &md5, string &filename) {
    ...
    (mysql操作) 
    e)如果redis有记录，mysql和redis都需要处理，删除相关记录(redis操作)
    f)用户文件数量-1(redis操作)
    g)删除用户文件列表数据(mysql操作)
    h)文件信息表(file_info)的文件引用计数count，减去1(mysql操作)
    i)如果count为0，说明没有用户引用此文件，需要在storage删除此文件(storage操作)
    */
    int ret = 0;
    char sql_cmd[SQL_MAX_LEN] = {0};
    char fileid[1024] = {0};
